(define (domain delivery)
	...
	(:control-variable const
		:bounds (and (>= ?value 1) (<= ?value 1))
	)
	...
	(:durative-action start-delivery
		:duration (and (>= ?duration 0.05) (<= ?duration 0.05))
		:condition (and
			(at start (first-time))
            (at end (timer-started))
            (at end  (drone1-fly))
            (at end  (drone2-fly))
	...
	(:durative-action timer
		:duration (and (>= ?duration 0.01) (<= ?duration 1000))
		:condition (and
			(at start  (delivery-ongoing))
			(at end (complete-delivery))
		)
		:effect (and
			(increase (time) (* (const) #t))
			(at start (timer-started))
			(at end (finish-timer))
		)
	)

		(:durative-action house2-on
		:duration (and (>= ?duration 0.01) (<= ?duration 20))
		:condition (and
			(at start  (>= (time) 160))
			(at end    (<= (time) 180))
		)
		:effect (and
			(at start (house2-available))
			(at end (not(house2-available)))			
		)
	)
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;	drone1 fly actions
